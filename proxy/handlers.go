...
func (s *ProxyServer) handleTCPSubmitRPC(cs *Session, id string, params []string) (bool, *ErrorReply) {
	...
	return s.handleSubmitRPCStratum(cs, cs.login, id, params)
}

func (s *ProxyServer) handleSubmitRPCStratum(cs *Session, login, id string, params []string) (bool, *ErrorReply) {
	...
	t := s.currentBlockTemplate()
	exist, validShare := s.processShareStratum(login, id, cs.ip, t, params)
...
func (s *ProxyServer) handleSubmitRPCGetwork(cs *Session, login, id string, params []string) (bool, *ErrorReply) {
	if !workerPattern.MatchString(id) {
		id = "0"
	}
	if len(params) != 3 {
	...
	t := s.currentBlockTemplate()
	exist, validShare := s.processShareGetwork(login, id, cs.ip, t, params)
	ok := s.policy.ApplySharePolicy(cs.ip, !exist && validShare)
