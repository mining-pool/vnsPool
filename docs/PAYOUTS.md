...
* Submit a transaction to a node via `vns_sendTransaction`
...
If there was a debit operation performed which is not followed by actual money transfer (after `vns_sendTransaction` returned an error), you will likely see:
